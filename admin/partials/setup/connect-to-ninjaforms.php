...
if( !class_exists('Ninja_Forms') ) {
    $message = __("Please install/activate to configure Ninja Forms connections with RAO Forms","raoforms");
    display_notice($class,$message);
    wp_die();
}
...
$ninjaforms_array = wp_list_pluck($ninja_forms,"form_title","id");
...
if(empty($rao_forms)) {
    display_rao_form_empty_notice("Ninja Forms");
    wp_die();
}
$rao_forms_array = array();

$get_connections = get_connections("ninjaforms");
...
?>
<input type="hidden" id="form_provider" value="ninjaforms" />
...
<div class="grid-container">
    <div class="provider-forms">
        <select id="provider-options" class="provider-options" name="provider-options">
            <?php
            echo '<option value="" selected disabled>'.__("Select NinjaForms Form","").'</option>';
            if( !empty($ninjaforms_array) ) {
                
                foreach( $ninjaforms_array as $key => $value ) {
...
<div class="connected-form-sections">
    ...
    <div class="page-title" <?php echo esc_attr($display_heading);?>>
        <h2><?php _e('NinjaForms Connections',"raoforms"); ?></h2>
    </div>
    <div class="connected-forms">
        ...
        $delete_connections = array();
            ...
            foreach($get_connections as $connection) :
                ...
                if(!isset($ninjaforms_array[$connection["provider_form_id"]])) {
                    $delete_connections[] = $connection["id"];
                    continue;
                    }
                    if(!isset($rao_forms_array[$connection["rao_form_id"]]))
                    continue;
            $provider_form_title = $ninjaforms_array[$connection["provider_form_id"]];
